...
#CJ changed to merge to fix issue with NA values
merged_data_2 <-merge(events, app_events, by = "event_id", allow.cartesian=TRUE)
...
#CJ changed to merge to fix issue with NA values
merged_data_3 <-merge(app_labels, label_categories, by = "label_id", allow.cartesian=TRUE)
...
#CJ changed to merge to fix issue with NA values
merged_data_4 <- merge(merged_data_1, merged_data_2, by = "device_id", allow.cartesian=TRUE)
...
#CJ changed to merge to fix issue with NA values
final_merged_data <- merge(merged_data_4, merged_data_3, by = "app_id", allow.cartesian=TRUE)
...
final_merged_data$category <- collapsecategory(final_merged_data$category, p = .01)
final_merged_data$big_category <- collapsecategory(final_merged_data$big_category, p = .01)
final_merged_data$label_id <- collapsecategory(final_merged_data$label_id, p = .01)
###Writing to csv file (note hashed out because I don't want to rewrite)
write_csv(final_merged_data, "merged_data_CJ2.4.17.csv")
