...
merged_data_3$high_flow = ifelse(merged_data_3$category== "High Flow", 1, 0)#Creates a flag for 773,High Flow,flag_1
...
#Creating variables for categories that may be predictive
final_merged_data$flight=ifelse(final_merged_data$label_id== 188, 1, 0)#Creates a flag for 188, Flight, which has high number of females 24-26 years old,flag_3
final_merged_data$take_away=ifelse(final_merged_data$label_id== 211, 1, 0)#Creates a flag for 211, Take-Away ordering, which has high number of females 24-26 years old,flag_4
...
#Removing the Labels from data to not duplicate
...
final_merged_data$high_mobility=ifelse(final_merged_data$label_id== 774, 1, 0)#Creates a flag for 774,High Mobility,flag_2
final_merged_data$laundry=ifelse(final_merged_data$label_id== 464, 1, 0)#Creates a flag for 464, laundry
final_merged_data$android=ifelse(final_merged_data$label_id== 631, 1, 0)#Creates a flag for 631, android
final_merged_data$mobile2G=ifelse(final_merged_data$label_id== 645, 1, 0)#Creates a flag for 645, 2G
final_merged_data$screen_sz=ifelse(final_merged_data$label_id== 621, 1, 0)#Creates a flag for 621, screen size
final_merged_data$unknown_318=ifelse(final_merged_data$label_id== 318, 1, 0)#Creates a flag for 318, unknown activity
final_merged_data$med_liquidity=ifelse(final_merged_data$label_id== 775, 1, 0)#Creates a flag for 775, Liquid Medium, which has high number of older people,flag_5
...
#Removing labels since we have new variables
final_merged_data$label_id <- ifelse(final_merged_data$label_id == 773, NULL, final_merged_data$label_id) #Corresponds to high_flow category above
final_merged_data$label_id <- ifelse(final_merged_data$label_id == 774, NULL, final_merged_data$label_id)
final_merged_data$label_id <- ifelse(final_merged_data$label_id == 631, NULL, final_merged_data$label_id)
final_merged_data$label_id <- ifelse(final_merged_data$label_id == 621, NULL, final_merged_data$label_id)
final_merged_data$label_id <- ifelse(final_merged_data$label_id == 318, NULL, final_merged_data$label_id)
final_merged_data$label_id <- ifelse(final_merged_data$label_id == 464, NULL, final_merged_data$label_id)
...
#Creating flags for labels that have different breakdown by age/gender group
final_merged_data$Finland =ifelse(final_merged_data$label_id== 463, 1, 0) # Creates a flag for 463
final_merged_data$label_id <- ifelse(final_merged_data$label_id == 463, NULL, final_merged_data$label_id) 

final_merged_data$industry_cat=ifelse(final_merged_data$category== 'Industry tag', 1, 0)#Creates a flag for Industry tag category
final_merged_data$category <- ifelse(final_merged_data$category == 'Industry tag', NULL, final_merged_data$label_id)

final_merged_data$High_risk=ifelse(final_merged_data$category== 'High risk', 1, 0)#Creates a flag for Industry tag category
final_merged_data$category <- ifelse(final_merged_data$category == 'High risk', NULL, final_merged_data$label_id)

final_merged_data$low_risk=ifelse((final_merged_data$category== 'Low risk'| final_merged_data$category== 'Low Risk'), 1, 0)#Creates a flag for Industry tag category
final_merged_data$category <- ifelse((final_merged_data$category== 'Low risk'| final_merged_data$category== 'Low Risk'), NULL, final_merged_data$label_id)

final_merged_data$Higher_risk=ifelse(final_merged_data$category== 'Higher risk', 1, 0)#Creates a flag for Industry tag category
final_merged_data$category <- ifelse(final_merged_data$category == 'Higher risk', NULL, final_merged_data$label_id)

final_merged_data$medium_risk=ifelse(final_merged_data$category== 'Medium risk', 1, 0)#Creates a flag for Industry tag category
final_merged_data$category <- ifelse(final_merged_data$category == 'Medium risk', NULL, final_merged_data$label_id)

final_merged_data$property=ifelse((final_merged_data$category== 'Property Industry 2.0'|final_merged_data$category== 'Property Industry 1.0'), 1, 0)#Creates a flag for property category
final_merged_data$category <- ifelse((final_merged_data$category== 'Property Industry 2.0'|final_merged_data$category== 'Property Industry 1.0'), NULL, final_merged_data$label_id)

final_merged_data$services=ifelse(final_merged_data$services== 'Services 1', 1, 0)#Creates a flag for services category
final_merged_data$category <- ifelse(final_merged_data$category == 'Services 1', NULL, final_merged_data$label_id)

final_merged_data$custom=ifelse(final_merged_data$category== 'Custom label', 1, 0)#Creates a flag for custom label category
final_merged_data$category <- ifelse(final_merged_data$category == 'Custom label', NULL, final_merged_data$label_id)
...
by_device_id_train_counts_1 <- final_merged_data_train %>%
  ...
  dplyr::summarize(
    ...
    high_flow = sum(high_flow, na.rm = TRUE), 
    flight = sum(flight, na.rm = TRUE), 
    med_liquidity = sum(med_liquidity, na.rm = TRUE),
    ...
    laundry = sum(laundry, na.rm = TRUE),
    android = sum(android, na.rm = TRUE),
    unknown_318 = sum(unknown_318, na.rm = TRUE),
    mobile2G = sum(mobile2G, na.rm = TRUE),
    High_risk = sum(High_risk, na.rm = TRUE),
    take_away = sum(take_away, na.rm = TRUE),
    video_tag = sum(video_tag, na.rm = TRUE),
    im_tag = sum(im_tag, na.rm = TRUE),
    Finland = sum(Finland, na.rm = TRUE),
    industry_cat = sum(industry_cat, na.rm = TRUE),
    property= sum(property, na.rm = TRUE),
    custom = sum(custom, na.rm = TRUE),
    screen_sz = sum(screen_sz, na.rm = TRUE)
  )
